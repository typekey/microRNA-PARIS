...
workflow FASTQ_FASTQC_UMITOOLS_CUTADAPT_paired2single {
    ...
    if (params.mirna_table) {
        ...
        keyed_mirna_ch
            .cross(keyed_reads_ch)
            .map { mirna_pair, read_pair ->
                def (read_key, read_meta, reads) = read_pair
                def (mirna_key, mirna_meta)       = mirna_pair

                def new_meta = read_meta + [
                    ...
                    single_end   : true,
                    id           : "${read_meta.id}_${mirna_meta.miRNA}"
                ]
