...
include { STAR_REPEAT_ALIGN              } from '../../../modules/local/star/repeat_align'
include { STAR_GENE_ALIGN              } from '../../../modules/local/star/gene_align'
...
workflow ALIGN_STAR {
    ...
    if (is_aws_igenome) {
        ...
        if (map_target == "repeat") {
            STAR_REPEAT_ALIGN ( reads, index, gtf, star_ignore_sjdbgtf, seq_platform, seq_center )
            ch_orig_bam       = STAR_REPEAT_ALIGN.out.bam
            ch_log_final      = STAR_REPEAT_ALIGN.out.log_final
            ch_log_out        = STAR_REPEAT_ALIGN.out.log_out
            ch_log_progress   = STAR_REPEAT_ALIGN.out.log_progress
            ch_bam_sorted     = STAR_REPEAT_ALIGN.out.bam_sorted
            ch_bam_transcript = STAR_REPEAT_ALIGN.out.bam_transcript
            ch_fastq          = STAR_REPEAT_ALIGN.out.fastq
            ch_tab            = STAR_REPEAT_ALIGN.out.tab
            ch_unmapped_1     = STAR_REPEAT_ALIGN.out.unmapped_1
            ch_unmapped_2     = STAR_REPEAT_ALIGN.out.unmapped_2
            ch_versions       = ch_versions.mix(STAR_REPEAT_ALIGN.out.versions.first())
        } else if (map_target == "gene") {
            STAR_GENE_ALIGN ( reads, index, gtf, star_ignore_sjdbgtf, seq_platform, seq_center )
            ch_orig_bam       = STAR_GENE_ALIGN.out.bam
            ch_log_final      = STAR_GENE_ALIGN.out.log_final
            ch_log_out        = STAR_GENE_ALIGN.out.log_out
            ch_log_progress   = STAR_GENE_ALIGN.out.log_progress
            ch_bam_sorted     = STAR_GENE_ALIGN.out.bam_sorted
            ch_bam_transcript = STAR_GENE_ALIGN.out.bam_transcript
            ch_fastq          = STAR_GENE_ALIGN.out.fastq
            ch_tab            = STAR_GENE_ALIGN.out.tab
            ch_unmapped_1     = STAR_GENE_ALIGN.out.unmapped_1
            ch_unmapped_2     = STAR_GENE_ALIGN.out.unmapped_2
            ch_versions       = ch_versions.mix(STAR_GENE_ALIGN.out.versions.first())
        } else {
            error "Unknown mapping target: ${map_target}"
        }
    }
    ...
    unmapped_1     = ch_unmapped_1
    unmapped_2     = ch_unmapped_2
